...
import com.fsbtech.interviews.rest.FSBRestControllerClient;
...
class FSBRestControllerClientTest {
    private FSBRestControllerClient fsbRestClient;
    ...
    @BeforeEach
    private void setup(){
        fsbRestClient = new FSBRestControllerClient();
        testEvents = addEvents(fsbRestClient);
    }

    private List<Event> addEvents(FSBRestControllerClient client) {
    ...
    void addEvent() {
        ...
        testEvents.stream().forEach(e -> fsbRestClient.addEvent(e));
        fsbRestClient.addEvent(testEvent);
        assertEquals(fsbRestClient.eventsMap.get(testId),testEvent);
    }
    ...
    void eventCompleted() {
        ...
        fsbRestClient.addEvent(testEvent);
        fsbRestClient.addEvent(testEvent2);
        Integer testId = testEvent2.getId();
        fsbRestClient.eventCompleted(testId);
        assertEquals(fsbRestClient.eventsMap.size(),1);
        assertTrue(fsbRestClient.eventsMap.get(testId)==null);
    }
    ...
    void attachMarketRefTypeToEvent() {
        ...
        fsbRestClient.addEvent(testEvent2);
        Integer testId = testEvent.getId();
        fsbRestClient.attachMarketRefTypeToEvent(testId, refType2);
        assertEquals(fsbRestClient.eventsMap.size(),2);
        assertTrue(fsbRestClient.eventsMap.get(testId).getMarketRefTypes().contains(refType2));
    }
    ...
    void removeMarketRefTypeFromEvent() {
        ...
        fsbRestClient.addEvent(testEvent2);
        Integer testId = testEvent2.getId();
        fsbRestClient.removeMarketRefTypeFromEvent(testId, refType2);
        assertEquals(fsbRestClient.eventsMap.size(),2);
        assertTrue(!fsbRestClient.eventsMap.get(testId).getMarketRefTypes().contains(refType2));
    }
    ...
    void futureEventNamesCollectionWithCategory() {
        testEvents.forEach(event -> fsbRestClient.addEvent(event));
        Collection<String> result = fsbRestClient.futureEventNamesCollection("Football", null, null);
    ...
    void futureEventNamesCollectionWithSubCategory() {
        testEvents.forEach(event -> fsbRestClient.addEvent(event));
        Collection<String> result = fsbRestClient.futureEventNamesCollection("", "Premier League", null);
    ...
    void futureEventNamesCollectionWithRefType() {
        testEvents.forEach(event -> fsbRestClient.addEvent(event));
        Collection<String> result = fsbRestClient.futureEventNamesCollection(null, "", "Home");
    ...
    void futureEventNamesCollectionWithAll() {
        testEvents.forEach(event -> fsbRestClient.addEvent(event));
        Collection<String> result = fsbRestClient.futureEventNamesCollection("Football", "Premier League", "Away");
        assertEquals(2, result.size());
    }
    ...
    void futureEventNamesCollectionWithSubCategoryAndRefType() {
        testEvents.forEach(event -> fsbRestClient.addEvent(event));
        Collection<String> result = fsbRestClient.futureEventNamesCollection(null, "Premier League", "Home");
        assertEquals(2, result.size());
    }
    ...
    void dumpFullStructure() {
        ...
        Integer testId = testEvent.getId();
        testEvents.stream().forEach(e -> fsbRestClient.addEvent(e));
        String output = fsbRestClient.dumpFullStructure();
